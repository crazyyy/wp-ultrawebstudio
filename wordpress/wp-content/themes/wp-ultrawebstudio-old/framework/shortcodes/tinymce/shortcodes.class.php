	...
	function formate_shortcode() {
		...
		if( isset( $swm_shortcodes ) && is_array( $swm_shortcodes ) ) {
			...
			foreach( $this->params as $pkey => $param ) {
				...
				switch( $param['type'] ) {
					...
					case 'textarea' :
						...
						// prepare
						$output  = $row_start;
						$output .= '<textarea rows="10" cols="30" name="' . $pkey . '" id="' . $pkey . '" class="swm-form-textarea swm-input">' . $param['std'] . '</textarea>' . "\n";
						$output .= $row_end;
			...
			if( isset( $swm_shortcodes[$this->popup]['child_shortcode'] ) ) {
				...
				$prow_start .= '<td><a href="#" id="form-child-add" class="button-secondary">' . $swm_shortcodes[$this->popup]['child_shortcode']['clone_button'] . '</a>' . "\n";
				...
				foreach( $this->cparams as $cpkey => $cparam ) {
					...
					switch( $cparam['type'] ) { 
						...
						case 'textarea' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<textarea rows="10" cols="30" name="' . $cpkey . '" id="' . $cpkey . '" class="swm-form-textarea swm-cinput">' . $cparam['std'] . '</textarea>' . "\n";
							$coutput .= $crow_end;
