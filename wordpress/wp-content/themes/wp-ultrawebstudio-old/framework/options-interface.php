...
function optionsframework_tabs() {
	...
	foreach ($options as $value) {
		...
		if ($value['type'] == "heading") {
			$jquery_click_hook = preg_replace('/[^a-zA-Z0-9._\-]/', '', strtolower($value['id']) );
			//$jquery_click_hook = "of-option-" . $jquery_click_hook;
			$menu .= '<li><a id="'.  esc_attr( $jquery_click_hook ) . '_tab" class="nav-tab" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '">' . $value['name'] . '</a></li> ';
		}
	}	
	$menu .= '</ul>';
...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Basic text input
		case 'text':
			$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="of-input swm-input1" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="text" value="' . esc_attr( $val ) . '" />';
			break;
		...
		case 'textarea':
			...
			$val = stripslashes( $val );
			$output .= '<textarea id="' . esc_attr( $value['id'] ) . '" class="of-input swm-textarea1" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" rows="' . $rows . '">' . esc_textarea( $val ) . '</textarea>';
			break;

		// Select Box
		case ($value['type'] == 'select'):
			$output .= '<select class="swm_admin_dropdown" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" id="' . esc_attr( $value['id'] ) . '">';
		...
		case "radio":
			...
			foreach ($value['options'] as $key => $option) {
				...
				$output .= '<input class="of-input of-radio" type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $key ) . '" '. checked( $val, $key, false) .' /><label class="swm-radio-label" for="' . esc_attr( $id ) . '">' . esc_html( $option ) . '</label>';
				$output .= '</div>';
			}
			break;
		...
		// Checkbox
		case "checkbox":
			$output .= '<input data-on="Show" data-off="Hide" id="' . esc_attr( $value['id'] ) . '" class="checkbox of-input checkbox_on_off" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" '. checked( $val, 1, false) .' />';
			$output .= '<a href="#" class="tipLeft field-desc-help" title="' . wp_kses( $explain_value, $allowedtags) . '"></a>';
			break;
		...
		case 'typography':
			...
			// Font Size
			$output .= '<select class="of-typography of-typography-size swm_admin_dropdown_small" name="' . esc_attr( $option_name . '[' . $value['id'] . '][size]' ) . '" id="' . esc_attr( $value['id'] . '_size' ) . '">';
			for ($i = 9; $i < 71; $i++) {
				$size = $i . 'px';
		...
		case "heading":
			...
			$jquery_click_hook = preg_replace('/[^a-zA-Z0-9._\-]/', '', strtolower($value['id']) );
			//$jquery_click_hook = "of-option-" . $jquery_click_hook;
			$menu .= '<li><a id="'.  esc_attr( $jquery_click_hook ) . '_tab" class="nav-tab" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '">' . esc_html( $value['name'] ) . '</a> </li>';
		...
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
			...
			if ( ( $value['type'] != "checkbox" ) && ( $value['type'] != "editor" ) && ( $value['type'] != "upload" ) && ($explain_value != '') ) {
				$output .= '<a href="#" class="tipLeft field-desc-help" title="' . wp_kses( $explain_value, $allowedtags) . '"></a>'."\n";
			}
			$output .= '</div>';
