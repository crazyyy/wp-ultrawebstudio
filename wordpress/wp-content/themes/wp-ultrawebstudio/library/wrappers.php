...
function theme_post_wrapper($args = '') {
	...
	?>
	<article<?php echo $id; ?> class="my-post my-article <?php echo $class; ?>">
                                <?php
if (!theme_is_empty_html($title)) {
	echo '<'.$heading.' class="my-postheader">'.$title.'</'.$heading.'>';
}
?>
                                <?php echo $thumbnail; ?><div class="my-postcontent my-postcontent-0 clearfix"><?php echo $content; ?></div>
...
function theme_simple_wrapper($args = '') {
	...
	echo "<div class=\"my-widget{$class}\"{$id}>";
	if (!theme_is_empty_html($title))
		echo '<' . $heading . ' class="my-widget-title">' . $title . '</' . $heading . '>';
	echo '<div class="my-widget-content">' . $content . '</div>';
	echo '</div>';
}
...
function theme_block_wrapper($args) {
	...
	$begin = <<<EOL
<div {$id}class="my-block{$class} clearfix">
