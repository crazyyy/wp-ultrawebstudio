...
function theme_update_styles() {
	...
	wp_register_style("css", 'http://fonts.googleapis.com/css?family=Ubuntu+Condensed&subset=latin', array(), false, "all");
...
function theme_get_metadata_icons($icons = '', $class = '') {
	...
	for ($i = 0; $i < count($icons); $i++) {
		...
		switch ($icon) {
			case 'date':
				$result[] = '<span class="my-postdateicon">' . sprintf(__('<span class="%1$s">Published</span> %2$s', THEME_NS),
								'date',
			...
			break;
			case 'author':
				$result[] = '<span class="my-postauthoricon">' . sprintf(__('<span class="%1$s">By</span> %2$s', THEME_NS),
								'author',
			...
			case 'category':
				...
				if (theme_strlen($categories) == 0)
					break;
				$result[] = '<span class="my-postcategoryicon">' . sprintf(__('<span class="%1$s">Posted in</span> %2$s', THEME_NS), 'categories', get_the_category_list(', ')) . '</span>';
				break;
			case 'tag':
				$tags_list = get_the_tag_list('', ', ');
				if (!$tags_list)
					break;
				$result[] = '<span class="my-posttagicon">' . sprintf(__('<span class="%1$s">Tagged</span> %2$s', THEME_NS), 'tags', $tags_list) . '</span>';
				break;
			case 'comments':
				...
				$result[] = '<span class="my-postcommentsicon">' . theme_ob_get_clean() . '</span>';
				break;
			case 'edit':
				...
				$result[] = '<span class="my-postediticon">' . theme_ob_get_clean() . '</span>';
				break;
		}
	}
	...
	if (theme_is_empty_html($result))
		return;
	return "<div class=\"my-post{$class}icons my-metadata-icons\">{$result}</div>";
}
...
function theme_404_content($args = '') {
	$args = wp_parse_args($args, array(
		...
		'focus_script' => '<script type="text/javascript">jQuery(\'div.my-content input[name="s"]\').focus();</script>'
			)
	);
	extract($args);
	theme_post_wrapper(
...
function theme_stylize_pagination($pagination) {
	if ($pagination) {
		$pagination = '<div class="my-pager">' . str_replace(array('current', 'dots'), array('current active', 'dots more'), $pagination) . '</div>';
	}
	return $pagination;
}
...
function theme_comment_reply_link_filter($link) {
	return str_replace('class=\'', 'class=\'my-button ', $link);
}
...
function theme_comment($comment, $args, $depth) {
	...
	switch ($comment->comment_type) :
		...
		<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
			<div class="my-comment <?php echo $comment->comment_type ?> clearfix" id="comment-<?php comment_ID() ?>">
    <div class="my-comment-avatar"><?php echo theme_get_avatar(array('id' => $comment, 'size' => 80)); ?></div>
    <div class="my-comment-inner">
        <div class="my-comment-header comment-meta commentmetadata"><?php printf(__('%s on ', THEME_NS), get_comment_author_link($comment->comment_ID)); ?>
...
<?php edit_comment_link(__('(Edit)', THEME_NS)); ?></div>
        <div class="my-comment-content comment-body"><?php if ($comment->comment_approved == '0') : ?>
    <em><?php _e('Your comment is awaiting moderation.', THEME_NS); ?></em>
    <br />
<?php endif; ?>
<?php comment_text(); ?></div>
        <div class="my-comment-footer reply"><?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => $args['max_depth']))); ?></div>
    </div>
</div>
			<?php
			break;
		...
		<li class="post pingback">
			<div class="my-comment <?php echo $comment->comment_type ?> clearfix">
				<div class="my-comment-content comment-body"><?php _e('Pingback:', THEME_NS); ?> <?php comment_author_link(); ?><?php edit_comment_link(__('(Edit)', THEME_NS), ' '); ?></div>
			</div>
			<?php
			break;
	endswitch;
