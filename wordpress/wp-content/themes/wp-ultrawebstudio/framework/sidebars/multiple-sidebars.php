...
class sidebar_generator {
	...
	function admin_menu(){
		add_theme_page('Sidebars', __('Multiple Sidebars', 'swmtranslate'), 'manage_options', 'multiple_sidebars', array('sidebar_generator','admin_page'));		
}
		...
		<div class="wrap">
			...
			<table class="widefat page" id="sbg_table" style="width:600px;">
				...
				<?php
					...
					?>
					<tr>
						<td colspan="3"><?php _e('No Sidebars defined', 'swmtranslate') ?></td>
					</tr>
					<?php
				}
				?>
            ...
            <div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar" class="button-primary"><?php _e('+ Add Sidebar', 'swmtranslate') ?></a>
		...
		<div id="sbg_box" class="postbox " >
			...
			<div class="inside">
				<div class="sbg_container">
					...
					<p><?php _e('Please select the sidebar you would like to display on this page. <strong>Note:</strong> You must first create the sidebar under Appearance > Multiple Sidebars.', 'swmtranslate') ?></p>
					<ul>
					<?php 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator_replacement[<?php echo $i?>]">
								<option value="0"<?php if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>><?php _e('None', 'swmtranslate') ?></option>
							<?php
