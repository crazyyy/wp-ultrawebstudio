	...
	class RW_Meta_Box
	{
		...
		public function show()
		{
			...
			foreach ( $this->fields as $field )
			{
				...
				######### swm_id_{$field['id']} added for show/hide ajax dropdown#####################################

				// Display label and input in DIV and allow user-defined classes to be appended
				$classes = array( 'rwmb-field', "rwmb-{$field['type']}-wrapper", "swm_id_{$field['id']}" );
		...
		static function end_html( $html, $meta, $field )
		{
			...
			/* ###############  NEW CLASS description FIELD ############### */

			$desc = ! empty( $field['desc'] ) ? " <div class='swm_meta_desc'><a href='#' id='{$id}_description' class='tipUp swm_meta_desc_help' title='{$field['desc']}'></a></div>" : '';

			

			/* ###############  NEW CLASS BELOW META FIELD ############### */


			$my_class = ! empty( $field['my_class'] ) ? "<div class='{$field['my_class']}'></div>" : '';

			// Closes the container
			$html = "{$button} </div>{$desc}<div class='clear'></div>{$my_class}";
...
####### inc/helpers.php - changed size thumb to full - line #208 ########
